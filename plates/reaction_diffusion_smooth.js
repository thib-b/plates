/**
 * Reaction-Diffusion Smooth Plate - Gray-Scott Model
 * High-resolution version with no visible grid
...
// Create offscreen graphics buffer for smooth rendering
let displayBuffer;
...
function setup() {
  ...
  // Create offscreen buffer for smooth rendering
...
function drawPlate() {
  ...
  // Use high-resolution rendering by drawing directly with smooth interpolation
  const cellSize = (plateRadius * 2) / gridSize;
  const offsetX = width / 2 - plateRadius;
  const offsetY = height / 2 - plateRadius;
  noStroke();

  // Draw with anti-aliasing effect by using slightly overlapping, semi-transparent cells
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      ...
      // Draw slightly larger cells to hide grid lines
      rect(offsetX + i * cellSize, offsetY + j * cellSize, cellSize + 1, cellSize + 1);
    }
  }
  
  // Show drawing indicator
  const indicator = document.getElementById('drawing-indicator');
  if (drawingMode && indicator) {
...
function keyPressed() {
  if (key === ' ') { 
    ...
    saveCanvas('reaction_diffusion_smooth_' + year() + nf(month(), 2) + nf(day(), 2) + '_' + nf(hour(), 2) + nf(minute(), 2) + nf(second(), 2), 'png');
  } else if (key === 'd' || key === 'D') { 
